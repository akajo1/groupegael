...
export default function WorshipItem(props) {
    const item = props.item;
    const theme = props.theme
  
    return (
       <TouchableOpacity onPress={()=> props.navigation.navigate(props.type,{item: item})}>
            <Animatable.View animation="fadeInRight" style={{width: width/1.9}}>
               
        <ImageLoad defaultImageSource={require('../assets/default-thumb.png')} source={{uri:'https://api.groupegael.com/storage/images/'+item.picture}} style={{width: width/2,height: height/4.2,borderRadius:5}} resizeMethod="auto" resizeMode="cover" />
